...
Current version: **2.3.0**. V2.3.0 introduced `I_EcSlaveRegistry` (`ConfiguredSlaveCount`, `GetSlaveIndexByAddr`, `GetSlaveName`, `IsHotConnectMember`), extended by `I_EtherCatMasterDiagnostic`; `SyncUnitTask` is now constructed with `(Name, Registry : I_EcSlaveRegistry)` instead of the master interface plus two buffer references (breaking only for direct `SyncUnitTask` users), and `I_EcIoDevice.Configuration` / `State` are Get-only on the interface. V2.2.0 requires **TwinCAT 3.1.4026.27 and `Tc2_EtherCAT` 3.8.2.0 or newer**: `EtherCatMaster` reads slave states with `FB_EcGetAllExtSlaveStates` and `I_EcIoDevice.State` is now `REFERENCE TO ST_EcExtendedSlaveState` (breaking). It also scaled the EtherCAT diagnostics to 2500 slaves on a 10 ms task (`SLAVES_PER_CYCLE` slicing, O(1) address map, `I_CoeTransferScheduler` so only CoE transfers in flight are serviced, `E_EcErrorSource`). See `Documentation/EtherCAT.md`. V2.0.0 added the MC3 axis stack and renamed all pre-existing motion types with an `Mc2` prefix (breaking change). V2.1.0 is also a breaking change and did three things: removed `AXIS_REF` and the motion-library enums from the *shared* axis abstraction (`I_Axis` / `I_Axis_PTP`) so the per-generation HMI/event function blocks could merge into `AxisPTP_HMI` and `Axis_TcEvents` — `I_Mc2Axis` / `I_Mc3Axis` still expose the raw `AXIS_REF`; extracted axis initialization into `Mc2AxisParameterLoader` / `Mc3AxisParameterLoader`; and made gearing a queryable `I_AxisGear` role, with every PTP command now refusing while the axis is `Coupled`. See `Documentation/Motion.md`.
